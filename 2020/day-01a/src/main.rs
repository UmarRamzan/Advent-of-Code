...
fn main() {
    ...
    let (num_one, num_two) = find_nums(&sorted_vec, 2020);
    let solution = num_one * num_two;
...
fn find_nums(input_vec: &Vec<i32>, expected_sum: i32) -> (i32, i32) {
    let mut i = 0;
    let mut j = input_vec.len() - 1;

    loop {
        let sum = input_vec[i] + input_vec[j];
        if sum == expected_sum {
            break;
